...
@property (weak, nonatomic) WhiteboardViewController * whiteboardVC;
...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(nullable id)sender {
    return self.whiteboardVC == nil ? YES : NO;
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(nullable id)sender {
    self.whiteboardVC = segue.destinationViewController;
    self.whiteboardVC.whiteboardEngine = [self.engineKit whiteboardEngine];
}
...
- (void)onWhiteboardAvailable {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self displayMessage:[NSString stringWithFormat:@"Whitboard available."]];
        self.whiteboardButton.enabled = YES;
    });
}

- (void)onWhiteboardUnavailable {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self displayMessage:[NSString stringWithFormat:@"Whitboard unavailable."]];
        [self closeWhitboard];
...
- (void)onWhiteboardStart {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self displayMessage:[NSString stringWithFormat:@"Whitboard start."]];
        [self openWhiteboard];
    });
}
...
- (void)onWhiteboardStop {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self displayMessage:[NSString stringWithFormat:@"Whitboard stop."]];
        [self closeWhitboard];
    });
}
...
- (void)openWhiteboard {
    if (self.whiteboardVC == nil) {
        [self performSegueWithIdentifier:@"OpenWhiteboard" sender:nil];
    }
}

- (void)closeWhitboard {
    if (self.whiteboardVC != nil) {
        [self.whiteboardVC close];
