...
let whiteboardInstance

joinBtn.onclick = async () => {
  if (whiteboardInstance) {
    onOpenWhiteboard(whiteboardInstance)
    return
  }
  ...
  const whiteboard = new Panortc.RtcWhiteboard()
  whiteboard.joinAndConnect({
    appId,
    panoToken,
    channelId,
    name,
    userId,
    onSuccess(rtcWhiteboard) {
      whiteboardInstance = rtcWhiteboard
      ...
      rtcWhiteboard.on(Panortc.RtcWhiteboard._WS_FAILOVER, reconnect)
      rtcWhiteboard.on(
      ...
      function reconnect() {
        console.error('handle whiteboard failover')
        setTimeout(() => {
          whiteboard.joinAndConnect({
            userId,
            appId,
            ...
            name,
            onSuccess: () => {
              console.log('failover reconnect success')
            },
            onFail: () => {
              console.log('failover reconnect failed')
              reconnect()
            }
          })
        }, 3000)
  ...
  whiteboard.on(Panortc.RtcWhiteboard.Events.openStateChanged, () =>
    onOpenWhiteboard(whiteboard)
  )
}
